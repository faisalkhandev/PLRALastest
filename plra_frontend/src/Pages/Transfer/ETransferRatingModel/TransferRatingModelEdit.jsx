...
import { useGetHRCalendarYearQuery } from "../../../Features/API/API";
import { useGettransferratingmodelbyidQuery, useGettransferratingtypesQuery, useGettransferratingtypesbyidQuery, usePutratingformulastypesMutation, usePuttransferratingmodelMutation } from "../../../Features/API/Transfer";
import Theme from "../../../Theme/Light__Theme";
import "../../table.css";
import { toast } from "react-toastify";

const TransferRatingModelEdit = () => {
    ...
    const { data: TransferRating, isLoading: TransferRatingLoading, isError: TransferRatingisError, error: TransferRatingError, refetch: TransferRatingRefectch, } = useGettransferratingmodelbyidQuery(id);
    const { data: TransferRatingType, isLoading: TransferRatingTypeLoading, isError: TransferRatingTypeisError, error: TransferRatingTypeError, refetch: TransferRatingTypeRefectch, } = useGettransferratingtypesbyidQuery(id);
    const [puttransferratingmodel] = usePuttransferratingmodelMutation();
    const [putratingformulastypes] = usePutratingformulastypesMutation();
    const [formData, setFormData] = useState({ rating_model_rec_id: "", description: "", active: "", year: "" });
    const [formData1, setFormData1] = useState({ marks1: "", marks2: "", marks3: "", marks4: "" })
    const [formData2, setFormData2] = useState({ id1: "", id2: "", id3: "", id4: "" })

    useEffect(() => {
        HRYearRefectch(); TransferRatingTypesRefectch(); TransferRatingRefectch(); TransferRatingTypeRefectch();
    }, [HRYearRefectch, TransferRatingTypesRefectch, TransferRatingRefectch, TransferRatingTypeRefectch])

    useLayoutEffect(() => {
        setFormData({ rating_model_rec_id: TransferRating?.results[0]?.rating_model_rec_id, description: TransferRating?.results[0]?.description, active: TransferRating?.results[0]?.active, year: TransferRating?.results[0]?.year })
        setFormData1({ marks1: TransferRatingType?.results[0]?.max_marks, marks2: TransferRatingType?.results[1]?.max_marks, marks3: TransferRatingType?.results[2]?.max_marks, marks4: TransferRatingType?.results[3]?.max_marks })
        setFormData2({ id1: TransferRatingType?.results[0]?.id, id2: TransferRatingType?.results[1]?.id, id3: TransferRatingType?.results[2]?.id, id4: TransferRatingType?.results[3]?.id })
    }, [TransferRating, TransferRatingType])
    ...
    const handleInputChange = (e) => {
        ...
        if (name === 'rating_model_rec_id' && parseInt(value) < 1) {
            toast.error(`Rating Model Id cannot be a negative value`, {
                position: "top-center",
                autoClose: 3000,
            });
            return;
    ...
    const handleSave = async (e) => {
        ...
        if (formData.rating_model_rec_id === "" || formData.description === "" || formData.year === "") {
            ...
            const res = await puttransferratingmodel({ formData, id });
            ...
            if (res.error?.status === 400) {
                ...
                toast.success("Transfer Rating Model Updated", {
                    position: "top-center",
                    autoClose: 1000,
                });
                ...
                try {
                    for (const rowData of tableData) {
                        const res = await putratingformulastypes({
                            formData: {
                                "tranfer_type_category": rowData?.tranfer_type_category,
                                "max_marks": rowData?.max_marks,
                                "model": rowData?.model,
                                "type": rowData?.type,
                            }, id: rowData?.id
                        });
                        if (res.error?.status === 400) {
                            ...
                            console.log(res);
                            toast.success("Transfer Rating Formulas Updated", {
                                position: "top-center",
                                autoClose: 1000,
                            });
                        }
        ...
        <>
            ...
            <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 4 }}>
                <Grid item xs={12} md={6} sx={{ gap: 1, display: "flex", flexDirection: "column" }}>
                    <InputField name="rating_model_rec_id" label="Rating Model Id" type="number" value={formData.rating_model_rec_id} fullWidth onChange={handleInputChange} disabled />
                    <SimpleDropDown name="year" label="Year" isShowIcon={true} value={formData.year} options={hr_year} sx={{ marginRight: 0 }} onChange={(event) => handleDropDownChange(event, "year")} disabled />
                </Grid>
                <Grid item xs={12} md={6} sx={{ gap: 3, display: "flex", flexDirection: "column" }}>
                    <Box>
                        <InputField name="description" label="Description" type="text" value={formData.description} fullWidth onChange={handleInputChange} disabled />
...
export default TransferRatingModelEdit