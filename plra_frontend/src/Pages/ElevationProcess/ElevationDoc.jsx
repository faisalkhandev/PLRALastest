...
function ElevationDoc() {
  ...
  const columns = [
    ...
    },
    {
      ...
      minWidth:180,
      headerName: 'Progression Approval Date'
    },
    {
      field: 'elevation_effective_date',
      minWidth:180,
      headerName: 'Progression Effective Date'
    }
  ];
    ...
    <div style={{ margin: "14px 30px 0 30px", height: "calc(100vh - 100px)" }} className='EmployeeTableBox'>
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
        <Box sx={{ display: "flex", alignItems: "center" }}>
          ...
          <Breadcrumb title="Progression" breadcrumbItem="Progression / Progression Document" />
        </Box>
      ...
      <Grid item xs={12}>
        <Grid container columnSpacing={6} sx={{ px: 2 }}>
          ...
          <Grid item xs={12} md={6} sx={{ display: "flex", flexDirection: "column", gap: 2, }}>
            <InputField label='Effective Date ' type='date' value={effectiveDate} onChange={(e) => setEffectiveDate(e.target.value)} mandatory />
          </Grid>
        </Grid>
      </Grid>
