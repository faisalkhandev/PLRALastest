...
import { useDisciplinaryProceedingApiQuery, useHRUserApprovalsPutMutation } from "../../../../../Features/API/DisciplinaryProceedings.js";
...
const ApprovalDetail = () => {
  ...
  const [ApprovalsPut] = useHRUserApprovalsPutMutation();
  useEffect(() => { refetch(); }, []);
  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      if (
        ...
        formD.append("attachment_of_personal_hearing_notice", formData.attachment);
      }
      if (formData.from_date) {
        formD.append("issuance_of_personal_hearing_notice_date", formData.from_date);
      }
    ...
    try {
      ...
      if (res?.data?.id) {
        ...
        setTimeout(() => { navigate("/approval/disciplinaryprocedingapproval/hruserapproval"); }, 3000);
      }
    } catch (error) {
              ...
              {data?.approving_authority?.id == user_id && data?.status === "Pending" && (
                <>
                  <Box sx={{ width: '100%', display: "flex", alignItems: "center", justifyContent: "space-between", gap: 2, mt: 2 }}>
                    ...
                    />
                    <InputField labelWidth="330px" name="from_date" type="date" label="Personal Hearing Notice Date" onChange={handleChange} value={formData?.from_date} min={getCurrentDate} />
                  </Box>
                  <Box sx={{ width: "100%", display: "flex", justifyContent: "end", mt: 2, gap: 2, }}>
                    <Button sx={{ backgroundColor: theme.palette.success[300], color: theme.palette.success[600], borderRadius: "4px", fontSize: "14px", fontWeight: 500, p: 2, }} onClick={() => handleButtonClick("Refer to Director HR", data)}>
        ...
        <Box sx={{ width: "350px" }}>
          <DialogContent>
            <Typography variant="body2" color="initial" ml={2} sx={{ fontSize: "16px", display: "flex", justifyContent: "start", alignItems: "center", gap: 1, ml: '-5px' }}>
              <Warning color={`${theme.palette.warning[600]}`} /> Do you want to Refer to Director HR?
            </Typography>
          </DialogContent>
