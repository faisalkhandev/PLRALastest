...
import { useDirHRApprovalsPutMutation, useDisciplinaryProceedingApiQuery, useInquiryOutcomesApiQuery, useInquiryTypeApiByIdQuery, useInquiryTypeApiQuery } from "../../../../../Features/API/DisciplinaryProceedings.js";
...
const ApprovalDetail = () => {
  ...
  const [formData, setformData] = useState({
    attachment: null, inquiry_name: "", inquiry_type: "", amount: "", start_date: "", end_date: "",
  });
  const [ApprovalsPut] = useDirHRApprovalsPutMutation();
  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      if (
        ...
        formD.append("attachment_of_final_order", formData.attachment);
      }
      if (actionPerformed === "Closed" && !formData.inquiry_name && !formData.inquiry_type) {
        ...
        formD.append("status", "Refer to DG");
      }
    ...
    try {
      ...
      if (res?.data?.id) {
        ...
        setTimeout(() => { navigate("/approval/disciplinaryprocedingapproval/dirhrapproval"); }, 3000);
      }
    } catch (error) {
        ...
        {DisciplineProceedingsData?.approvals.map((data, index) => (
          <Accordion key={index}>
            ...
            <AccordionDetails>
              ...
              {data?.approving_authority?.id == user_id && data?.status === "Pending" && !data?.attachment_of_final_order && (
                <Box sx={{ display: "flex", alignItems: "start", p: 1, mt: 2, width: "50%" }}>
                  <InputField
                    name="attachment"
                    label="Attachment of Final Order"
                    type="file"
                    mandatory
                    labelWidth={'210px'}
              ...
              {data?.approving_authority?.id == user_id && data?.status === "Pending" && (
                <Grid container spacing={2} sx={{ mt: 0.5, display: "flex", flexDirection: "row" }}>
                  <Grid item xs={6} sx={{ display: "flex", flexDirection: "column", }}>
                    <SimpleDropDown name="inquiry_name" label="Inquiry Name" isShowIcon={true} value={formData.inquiry_name} options={inquiry_outcome} sx={{ marginRight: 2 }} onChange={(event) => handleDropDownChange(event, "inquiry_name")} />
                  </Grid>
                  <Grid item xs={6} sx={{ display: "flex", flexDirection: "column", }}>
                    <SimpleDropDown name="inquiry_type" label="Inquiry Type" isShowIcon={true} value={formData.inquiry_type} options={inquiry_type} sx={{ marginRight: 2 }} onChange={(event) => handleDropDownChange(event, "inquiry_type")} />
                  </Grid>
                </Grid>
              )}
              ...
              {data?.approving_authority?.id == user_id && data?.status === "Pending" && InquiryOutcomeIdData?.results[0]?.id === type_id && InquiryOutcomeIdData?.results[0]?.nature === "Amount" && (
                <Grid container spacing={0} sx={{ width: "100%", mt: 4 }}>
                  <Grid item xs={6} sx={{ display: "flex", flexDirection: "column", gap: 2, }}>
                    <InputField name="amount" label="Amount" onChange={handleChange} value={formData.amount} type="number" />
                  </Grid>
                </Grid>
              )}
              ...
              {data?.approving_authority?.id == user_id && data?.status === "Pending" && InquiryOutcomeIdData?.results[0]?.id === type_id && InquiryOutcomeIdData?.results[0]?.nature === "Date" && (
                <Grid container spacing={0} sx={{ width: "100%", mt: 4 }}>
                  <Grid item xs={6} sx={{ display: "flex", flexDirection: "column", gap: 2, }}>
                    <InputField name="start_date" type="date" label="Start Date" onChange={handleChange} value={formData?.start_date} min={getCurrentDate} />
                  </Grid>
                  <Grid item xs={6} sx={{ display: "flex", flexDirection: "column", gap: 2, }}>
                    <InputField name="end_date" type="date" label="End Date" onChange={handleChange} value={formData?.end_date} min={getCurrentDate} />
                  </Grid>
                </Grid>
              )}
        ...
        <Box sx={{ width: "350px" }}>
          <DialogContent>
            <Typography variant="body2" color="initial" ml={2} sx={{ fontSize: "16px", display: "flex", justifyContent: "start", alignItems: "center", gap: 1, ml: '-5px' }}>
              <Warning color={`${theme.palette.warning[600]}`} /> Do you want to {actionPerformed === "Closed" ? "close this case" : "refer to DG"}?
            </Typography>
          </DialogContent>
