...
import { useAllDirHRPendingApprovalQuery, useAllDirHRrHistoryApprovalQuery } from "../../../../../Features/API/DisciplinaryProceedings";
import DisciplinaryProcedingDialog from "../DisciplinaryProcedingDialog";
import { gridCellStyle } from "../../../../../Utils/cellstyle";

const AllDirHRApprovals = () => {
  ...
  console.log(user_id);
  const { data: pendingData, isLoading: pendingLoading, isError: PendingisError, error: Pendingerror, refetch: pendingRefetch, } = useAllDirHRPendingApprovalQuery(user_id);
  console.log("pendingData", pendingData);
  const { data: historyData, isLoading: historyLoading, isError: historyisError, error: historyerror, refetch: historyRefetch, } = useAllDirHRrHistoryApprovalQuery(user_id);
  ...
  const pendingcolumns = [
    {
      ...
      renderCell: (params) => {
        return <Link to={`/approval/dirhrapproval/${params?.row?.id}/dirhrapproval/${params?.row?.processid}`} style={{ color: "#379237", textDecoration: "underline" }}>
    ...
    <div>
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", }}>
        <Box sx={{ display: "flex", alignItems: "center" }}>
          ...
          <Breadcrumb title="Director HR Approvals" breadcrumbItem="Approvals / Director HR Approvals" />
        </Box>
...
export default AllDirHRApprovals;
