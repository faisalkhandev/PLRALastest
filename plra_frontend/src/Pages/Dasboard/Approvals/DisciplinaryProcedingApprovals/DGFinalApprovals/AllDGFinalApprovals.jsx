...
import { useAllDGFinalPendingApprovalQuery, useAllDGFinalrHistoryApprovalQuery } from "../../../../../Features/API/DisciplinaryProceedings";
import DisciplinaryProcedingDialog from "../DisciplinaryProcedingDialog";
import { gridCellStyle } from "../../../../../Utils/cellstyle";

const AllDGFinalApprovals = () => {
  ...
  console.log(user_id);
  const { data: pendingData, isLoading: pendingLoading, isError: PendingisError, error: Pendingerror, refetch: pendingRefetch, } = useAllDGFinalPendingApprovalQuery(user_id);
  console.log("pendingData", pendingData);
  const { data: historyData, isLoading: historyLoading, isError: historyisError, error: historyerror, refetch: historyRefetch, } = useAllDGFinalrHistoryApprovalQuery(user_id);
  ...
  const pendingcolumns = [
    {
      ...
      renderCell: (params) => {
        return <Link to={`/approval/dgfinalapproval/${params?.row?.id}/dgfinalapproval/${params?.row?.processid}`} style={{ color: "#379237", textDecoration: "underline" }}>
...
export default AllDGFinalApprovals;
