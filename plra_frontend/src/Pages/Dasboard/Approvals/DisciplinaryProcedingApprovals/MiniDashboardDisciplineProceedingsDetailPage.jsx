...
const MiniDashboardDisciplineProceedingsDetail = ({ onCasualClick, leaveId }) => {
                    ...
                    <Grow in={detail} timeout={{ enter: 800, exit: 1000 }}>
                        <Box sx={{ display: 'flex', alignItems: "center", justifyContent: "space-between", px: 0.5 }}>
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                                <Typography variant="body2" color="initial" fontWeight={600} sx={{lineHeight:'4px'}}>Center</Typography>
                                <Typography variant="body2" color="initial" fontWeight={600} sx={{lineHeight:'4px'}} >Position</Typography>
                                <Typography variant="body2" color="initial" fontWeight={600} sx={{lineHeight:'4px'}} >Reporting Officer</Typography>

                            </Box>
                            <Box sx={{ textAlign: "right", display: 'flex', flexDirection: 'column', gap: 2}}>
                                <Typography variant="body2" color="initial" sx={{lineHeight:'4px'}} >{employeeData?.results[0]?.employee_id?.center?.center_name}</Typography>
                                <Typography variant="body2" color="initial" sx={{lineHeight:'4px'}} >{employeeData?.results[0]?.employee_id?.position?.job?.job_title}</Typography>
                                <Typography variant="body2" color="initial" sx={{lineHeight:'4px'}} >{employeeData?.results[0]?.employee_id?.reporting_officer?.first_name + ' ' + employeeData?.results[0]?.employee_id?.reporting_officer?.last_name}</Typography>
                            </Box>
                        </Box>
                    </Grow>
                </Box>
...
export default MiniDashboardDisciplineProceedingsDetail;
