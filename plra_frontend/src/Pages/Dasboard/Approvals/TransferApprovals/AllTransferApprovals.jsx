...
import { useGetAllTransferHistoryQuery, useGetAllTransferPendingQuery } from "../../../../Features/API/Transfer";
import TransferDialog from "./TransferDialog";

const AllTransferApprovals = () => {
  ...
  console.log(user_id);
  const { data: pendingData, isLoading: pendingLoading, isError: PendingisError, error: Pendingerror, refetch: pendingRefetch, } = useGetAllTransferPendingQuery(user_id);
  console.log("pendingData", pendingData);
  const { data: historyData, isLoading: historyLoading, isError: historyisError, error: historyerror, refetch: historyRefetch, } = useGetAllTransferHistoryQuery(user_id);
  ...
  const pendingRecord = pendingData?.results?.map((approval) => ({
    id: approval?.id,
    emp_name: approval?.transfer_process?.employee?.first_name +" " + approval?.transfer_process?.employee?.last_name,
    comments: approval.comments,
    visible: approval.visible,
    status: approval.status,
    regisnation_id: approval.resignation_request,
  })) || [];
  ...
  const pendingcolumns = [
    {
      field: "regisnation_id",
      headerName: "Transfer Id",
      minWidth: 200,
      renderCell: (params) => {
        return (
          <Link
            to={`/approval/transferapproval/${params?.row?.id}`}
  ...
  const handleRowClick = async (params) => {
    ...
    const data = await axios.get(
      `http://127.0.0.1:8000/Administrative_transfer/transfer-approvals/${params?.row?.id}`
    );
    ...
    <div>
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center" }}>
          ...
          <Breadcrumb
            ...
            breadcrumbItem="Approvals / TransferApprovals"
          />
        </Box>
      ...
      <Dialog open={dialogOpen} onClose={handleCloseDialog}>
        <Box sx={{ width: "800px", p: 2, height: "70vh", overflow: "scroll" }}>
          ...
          <TransferDialog DialogData={selectedHistory} />
        </Box>
      </Dialog>
    </div>
  );
...
export default AllTransferApprovals;
