...
const SuperApprovalPendingDetail = () => {
    ...
    return (
        <Box sx={{ p: 1 }}>
            ...
            {/* super approvals */}
            <>
            <Typography variant="h6" sx={{ my: 2 }}>Alternate Approval:</Typography>
            {leaveData?.results[0]?.superapprovals.map((data, index) => (
                <Accordion key={index} sx={{ p: 1 }}>
                    <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls={`super-panel${index + 1}-content`} id={`super-panel${index + 1}-header`}>
                        <Box sx={{ width: '50%', display: 'flex', alignItems: 'center', gap: 2 }}>
                            <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>{data?.designation}</Typography>
                            <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>|</Typography>
                            <Typography variant="subtitle1" sx={{
                                fontWeight: 600,
                                px: 2,
                                borderRadius: "4px",
                                backgroundColor: data.status === "Pending" ? theme.palette.warning[300] : "black",
                                color: data.status === "Pending" ? theme.palette.warning.main : "black"
                            }}>
                                {data.status}
                            </Typography>
                        </Box>
                    ...
                    <Grid container spacing={0}>
                        <Grid item xs={3} sx={{ border: "1px solid #E4E4E4", p: 1 }}>
                            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "start" }}>
                                <Typography variant="subtitle1" sx={{ fontWeight: 600, width: "150px" }}>Officer Name:</Typography>
                            </Box>
                        </Grid>
                        <Grid item xs={9} sx={{ border: "1px solid #E4E4E4", p: 1 }}>
                            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "start" }}>
                        ...
                        <Grid item xs={12} sx={{ p: 1, mt: 3,display: "flex", alignItems: "start", justifyContent: "start", gap:4  }}>
                        ...
                        <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'end', mt: 2, gap: 2 }}>
                            <Button
                                sx={{ backgroundColor: theme.palette.success[300], color: theme.palette.success.main, borderRadius: "4px", fontSize: "14px", fontWeight: 500, p: 2 }}
                                onClick={() => handleButtonClick("Approved", data)}
                            >
                                Accept
                            </Button>
                            <Button
                                sx={{ backgroundColor: theme.palette.error[300], color: theme.palette.error.main, borderRadius: "4px", fontSize: "14px", fontWeight: 500, p: 2 }}
            ...
            <Dialog open={reasonDialogOpen} onClose={handleCloseReasonDialog}  >
                <Box sx={{ width: '600px' }}>
                    ...
                    <DialogActions sx={{ mt: 4 }}>
                        ...
                        <Button onClick={()=>{setReasonDialogOpen(false)}} sx={{ borderRadius: "4px", backgroundColor: theme.palette.error[300], color: theme.palette.error[600], fontSize: "14px", fontWeight: 500, p: 2 }}>Cancel</Button>
                    </DialogActions>
                </Box>
            </Dialog>
        </Box >
...
export default SuperApprovalPendingDetail
