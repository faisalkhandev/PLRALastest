...
import { useHocoAllApprovalsQuery, useHocohistoryAllApprovalsQuery } from "../../../../../../Features/API/AnnualAssessment";
...
const AllCounterApprovals = () => {
  ...
  const {data: pendingData,isLoading: pendingLoading,isError: PendingisError,error: Pendingerror,refetch: pendingRefetch,} = useHocoAllApprovalsQuery(user_id);
  const {data: historyData,isLoading: historyLoading,isError: historyisError,error: historyerror,refetch: historyRefetch,} = useHocohistoryAllApprovalsQuery(user_id);
  ...
  const pendingcolumns = [
    {
      ...
      renderCell: (params) => {
        return <Link to={`/approval/annualassesment/${params?.row?.id}/hoc_approval/${params?.row?.processid}`} style={{ color: "#379237", textDecoration: "underline" }}>
    ...
    <div>
      <Box sx={{display: "flex",alignItems: "center",justifyContent: "space-between",}}>
        <Box sx={{ display: "flex", alignItems: "center" }}>
          ...
          <Breadcrumb title="Countersigning Officer Approvals" breadcrumbItem="Approvals / Countersigning Officer Approvals"/>
        </Box>
