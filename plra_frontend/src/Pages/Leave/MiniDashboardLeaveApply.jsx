...
const MiniDashboardLeaveApply = ({  empId }) => {
  const theme = useTheme();
  const [detail, setDetail] = useState(true);

  console.log(leaveId);
  const {data: employeeData,isLoading,refetch: employeeRefetch,} = useGetEmployeeDataByIDQuery(empId);
  ...
  return (
    <Box sx={{}}>
      ...
      <Typography variant="h5" color="initial" sx={{textAlign: "center",color: theme.palette.primary.main,mt: 1,fontWeight: 600,}}>
        {`${employeeData?.results[0]?.employee_id.title || ""} ${employeeData?.results[0]?.employee_id?.first_name || ""} ${employeeData?.results[0]?.employee_id?.last_name || ""}`}
      </Typography>
          ...
          <Grow in={detail} timeout={{ enter: 800, exit: 1000 }}>
            <Box sx={{display: "flex",alignItems: "center",justifyContent: "space-between",px: 0.5,}}>
              ...
              <Box sx={{textAlign: "right",display: "flex",flexDirection: "column",gap: 1,}}>
                ...
                <Typography variant="body2" color="initial">
                  {employeeData?.results[0]?.employee_id?.position?.position_id}
                </Typography>
                <Typography variant="body2" color="initial">
...
export default MiniDashboardLeaveApply;
