...
const EmployeeMinidashboard = ({ onCasualClick, leaveId }) => {
    ...
    console.log('leaveDataMiniEmployee: ', leaveData)
    ...
    const id = empId;
    const { data: employeeData, isLoading, refetch: employeeRefetch } = useGetEmployeeDataByIDQuery(id);
        ...
        <Box sx={{}}>
            ...
            <Typography variant="h5" color="initial"
                sx={{
                    ...
                    mt: 1, fontWeight: 600
                }}> {`${employeeData?.results[0]?.employee_id.title || ''} ${employeeData?.results[0]?.employee_id?.first_name || ''} ${employeeData?.results[0]?.employee_id?.last_name || ''}`}</Typography>
            <Box sx={{
                    ...
                    <Grow in={detail} timeout={{ enter: 800, exit: 1000 }}>
                        <Box sx={{ display: 'flex', alignItems: "center", justifyContent: "space-between", px: 0.5 }}>
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                                ...
                                <Typography variant="body2" color="initial" fontWeight={600}>Reporting Offficer</Typography>

                            </Box>
                            <Box sx={{ textAlign: "right", display: 'flex', flexDirection: 'column', gap: 1 }}>
                                <Typography variant="body2" color="initial" >{employeeData?.results[0]?.employee_id?.center?.center_name}</Typography>
                                <Typography variant="body2" color="initial" >{employeeData?.results[0]?.employee_id?.position?.position_id}</Typography>
                                <Typography variant="body2" color="initial" >{employeeData?.results[0]?.employee_id?.reporting_officer?.first_name + ' ' + employeeData?.results[0]?.employee_id?.reporting_officer?.last_name}</Typography>
                            </Box>
                        </Box>
                    </Grow>
                </Box>
