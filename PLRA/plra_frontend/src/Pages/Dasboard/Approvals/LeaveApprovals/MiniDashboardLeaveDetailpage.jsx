...
const MiniDashboardLeaveDetail = ({ fillter, onCasualClick, userID }) => {
  ...
  const { data: responseData, isError, Loader, refetch } = useGetCasualMedicalLeaveQuery({ fillter, userID });
  const filterData = responseData ? responseData.results : [];


  console.log("API Filter Data ", fillter , filterData);
    ...
    <>
          ...
          <Box
            ...
            <Box
              ...
              <Grid container spacing={0}>
                ...
                <Grid item xs={1.5} sx={{ textAlign: 'center' }}>Apply Date</Grid>
              ...
              {filterData?.map((record, id) => (
                ...
                >
                  ...
                  <Grid key={id} container spacing={0} >
                    <Grid item xs={1} sx={{ textAlign: 'center' }}>{record?.leave_request_id || " "}</Grid>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        <Box sx={{ width: '800px', p: 2, height: "70vh", overflow: 'scroll' }}>
          ...
          <LeaveDialog leaveData={selectedRecord} DialogData={selectedRecord} />
        </Box>
      </Dialog>
    </>
  );
...
export default MiniDashboardLeaveDetail;
