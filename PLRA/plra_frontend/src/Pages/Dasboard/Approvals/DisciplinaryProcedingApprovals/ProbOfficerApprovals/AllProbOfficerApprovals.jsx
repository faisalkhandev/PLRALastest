...
const AllProbOfficerApprovals = () => {
  ...
  console.log(user_id);
  const { data: pendingData, isLoading: pendingLoading, isError: PendingisError, error: Pendingerror, refetch: pendingRefetch, } = useAllProbeOfficerPendingApprovalQuery(user_id);
  console.log("pendingData", pendingData);
  const { data: historyData, isLoading: historyLoading, isError: historyisError, error: historyerror, refetch: historyRefetch, } = useAllProbeOfficerHistoryApprovalQuery(user_id);
  ...
  const pendingcolumns = [
    {
      ...
      renderCell: (params) => {
        return <Link to={`/approval/probofficerapproval/${params?.row?.id}/probofficerapproval/${params?.row?.processid}`} style={{ color: "#379237", textDecoration: "underline" }}>
    ...
    <div>
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", }}>
        <Box sx={{ display: "flex", alignItems: "center" }}>
          ...
          <Breadcrumb title="Probe Officer Approvals" breadcrumbItem="Approvals / Probe Officer Approvals" />
        </Box>
...
export default AllProbOfficerApprovals;
