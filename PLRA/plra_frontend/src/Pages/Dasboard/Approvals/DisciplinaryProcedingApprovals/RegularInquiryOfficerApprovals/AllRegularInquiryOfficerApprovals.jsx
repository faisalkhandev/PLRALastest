...
import { useAllRegularInquiryOfficerHistoryApprovalQuery, useAllRegularInquiryOfficerPendingApprovalQuery } from "../../../../../Features/API/DisciplinaryProceedings";
import DisciplinaryProcedingDialog from "../DisciplinaryProcedingDialog";
import { gridCellStyle } from "../../../../../Utils/cellstyle";

const AllRegularInquiryOfficerApprovals = () => {
  ...
  console.log(user_id);
  const { data: pendingData, isLoading: pendingLoading, isError: PendingisError, error: Pendingerror, refetch: pendingRefetch, } = useAllRegularInquiryOfficerPendingApprovalQuery(user_id);
  console.log("pendingData", pendingData);
  const { data: historyData, isLoading: historyLoading, isError: historyisError, error: historyerror, refetch: historyRefetch, } = useAllRegularInquiryOfficerHistoryApprovalQuery(user_id);
  ...
  const pendingcolumns = [
    {
      field: "id", headerName: "Id", minWidth: 150, renderCell: renderNullInRed,
      renderCell: (params) => {
        return <Link to={`/approval/regularinquiryofficerapproval/${params?.row?.id}/regularinquiryofficerapproval/${params?.row?.processid}`} style={{ color: "#379237", textDecoration: "underline" }}>
    ...
    <div>
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", }}>
        <Box sx={{ display: "flex", alignItems: "center" }}>
          ...
          <Breadcrumb title="Regular Inquiry Officer Approvals" breadcrumbItem="Approvals / Regular Inquiry Officer Approvals" />
        </Box>
...
export default AllRegularInquiryOfficerApprovals;
